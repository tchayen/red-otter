...
- **Immediate mode GUI** - the UI is a set of instructions that are executed every frame. This is how many game engines render their interfaces. The main benefit is extreme expressiveness and direct control over the rendering process. The main drawback is that it's hard to avoid unnecessary work and if for a complex UI the programmer decides to implement some caching, it's non-trivial and might end up being a retained mode UI in a trench coat. Complex layouts are often hard if the API

> IM is just an API paradigm. If you prefer it, then you can use it. You suffer no consequences for this. There is no such thing as something an IM can't do but an RM can.
> [Source](https://x.com/cmuratori/status/1733244416125747629?s=20)
...
[Source](https://github.com/ocornut/imgui/issues/97)
