  ...
  <body>
    <div id="content">
      ...
      <div class="note-block">
        <p>
          Using JSX syntax is entirely optional and if you don't want to use it,
          you will actually save yourself a lot of potential trouble with
          tricking your IDE to understand it and possibly you can also avoid
          stepping into uncharted territories of making React and another
          JSX-based library live in one project.
        </p>
        <p>
          If you want to see comparison of JSX and non-JSX syntax, see
          <a href="/#why-jsx">Why JSX</a> section.
        </p>
      </div>
      <p>
        For editor to correctly highlight JSX, add to <code>compilerOptions</code> in <code>tsconfig.json</code>:
      </p>
      ${codeExample(
        `{
  compilerOptions: {
    ...
    "jsxImportSource": "red-otter"
      ...
      <p>
        You will likely need to configure your bundler to enable JSX (in case of
        React or Preact this is usually done automatically with some plugin).
      </p>
      ${addHeader(3, "Vite")}
      <p>Add following to your <code>vite.config.ts</code>:</p>
      ${codeExample(
...
export default defineConfig({
  ...
  plugins: [jsx()],
        ...
        { language: "typescript", fileName: "vite.config.ts" }
      )}
      ${addHeader(3, "Parcel")}
      <p>
        Make sure that your <code>.parcelrc</code> config supports static files
        via raw transformer:
      </p>
      ${codeExample(
        `{
  ...
  "transformers": {
    "url:*": [
      "@parcel/transformer-raw"
        ...
        { language: "json", fileName: ".parcelrc" }
      )}
      <p>
        And then use a special import (in this example runtime loading, but the
        same principle applies to pregenerated data):
      </p>
      ${codeExample(
...
const file = await fetch(fontURL);`,
      ...
      <p>
        ...
        <code>&lt;canvas&gt;</code> element, load font (either with pregenerated
        data or in runtime, see <a href="/#generating-font-atlas">guide</a>),
        initiatilize <code>Context</code> and if you want to just draw shapes
        and polygons that's it. If you want to utilize layout engine (you
        probably do), then you need to add <code>Layout</code> and use one of
        available syntaxes to declare elements (see
        <a href="/#why-jsx">Why JSX</a> for comparison).
      </p>
      ${codeExample(
  ...
  <view style={container}>
    <view style={{ backgroundColor: zinc[900] }}>
      ...
      <ul>
        ...
        <li>
          Missing layout features: <code>margin</code>, <code>flex-wrap</code>, <code>flex-grow</code>, <code>flex-shrink</code>, <code>justify-content: space-around</code> and <code>space-evenly</code>, <code>overflow: hidden</code>, <del><code>aspect-ratio</code></del>.
        </li>
        <li>
...
*/
      ...
      <ul>
        <li>
          ...
          hidden</code>, <del><code>aspect-ratio</code></del>.
        </li>
        <li>
          <strong>Styling</strong>: <del><code>border-radius</code></del>,
          <del><code>border</code></del>, <code>box-shadow</code>,
          <code>opacity</code>.
        </li>
        <li>
